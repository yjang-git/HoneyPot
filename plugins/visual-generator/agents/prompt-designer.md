...
### Content Placement
{CONTENT 블록의 텍스트가 이미지 내 어디에 배치되는지 자연어로 설명}
- {영역 1}: {해당 영역에 배치될 텍스트와 시각 요소 설명}
- {영역 2}: {해당 영역에 배치될 텍스트와 시각 요소 설명}
...
| 카테고리 | 금지 단어/표현 |
|----------|---------------|
| **역할 라벨** | 메인 타이틀, 서브 타이틀, 핵심 모듈명, 핵심 메시지, 기능 설명, 입력 라벨, 출력 라벨, 적용 사례, 세부기술, 공통 요소, 핵심 비전, 주요 연구 분야, 정량적 성과 |
| **영역 라벨** | 중앙 코어, 좌측 상단 모듈, 좌측 하단 모듈, 우측 영역, 입력부 상단, 입력부 하단, 출력부, 비전 박스, 연구영역 N, 기대효과 N |
| **구성 지시** | Main Title, Sub-header, Section A, Large KPI, Hero, CTA |
| **포맷 힌트** | (강조), (볼드), (대형), (소형), (중앙배치) |

### 올바른 배치 정보 처리 방법

텍스트의 배치 위치와 역할은 **INSTRUCTION 블록의 ### Content Placement** 서브섹션에 자연어로 기술합니다:
...
- 중앙에서 좌측/우측으로 연결선, 우측에서 적용 사례로 분기하는 구조
```
...
### Content Placement
- 상단 배너: "제조업 디지털 전환 선도" 비전 선언문 배치
- 중앙 3개 직각 박스: 각 박스에 연구영역 제목(AI 품질 예측 모델, 실시간 공정 최적화, 디지털 트윈 플랫폼) 배치. 박스 헤더는 주조색 배경, 본문은 흰색 배경
- 하단 주석 영역: 기대효과 3개 항목(불량률 30% 감소, 생산성 25% 향상, 에너지 효율 20% 개선) 배치
- 시각 요소: 비전 박스에서 연구영역으로 방사형 연결선, 연구영역에서 기대효과로 하향 화살표, 각 연구영역 박스에 관련 아이콘(AI, 공정, 플랫폼)
