...
[Phase 0: 입력 검증 및 초기화]
    ...
    |   +-- layout 유효성 검증 (24종 참조: plugins/visual-generator/skills/layout-types/SKILL.md)
    |   +-- style 유효성 검증 (concept, gov, seminar)
    |
    +-- Step 0-2. 출력 디렉토리 구조 생성
        +-- {output_folder}/analysis/         # 문서 분석 결과
        +-- {output_folder}/prompts/          # 생성된 프롬프트
        +-- {output_folder}/images/           # 렌더링된 이미지
        +-- {output_folder}/reports/          # 실행 보고서
...
[Phase 3: 프롬프트 생성 - prompt-designer]
    ...
    +-- Step 3-3. 품질 검증
        ...
        +-- 4-block 구조 검증 (VISUAL_SPEC, CONTENT, STYLE, OUTPUT)
        +-- 미달 시 재생성 요청 (최대 2회)
